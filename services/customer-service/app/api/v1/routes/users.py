...
async def register(payload: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    await rabbitmq_manager.publish(
        "user_events",
        "user.registered",
        {"user_id": str(user.id), "email": user.email, "nombre": user.nombre, "rol": user.rol},
    )
...
async def login(payload: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    await redis.setex(
        ...
        json.dumps({"user_id": str(user.id), "email": user.email, "rol": user.rol}),
    )
...
async def update_profile(
    ...
    await rabbitmq_manager.publish(
        "user_events",
        "user.updated",
        {"user_id": str(user.id), "email": user.email, "nombre": user.nombre},
    )
